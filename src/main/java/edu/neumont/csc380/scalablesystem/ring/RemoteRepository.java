...
public class RemoteRepository implements RxHallaStor {
    ...
    protected static Single<Response> writeRequest(Socket connection, Request request) {
    ...
    private static Socket getSocket(String host, int port) {
        ...
        while (connection == null) {
            try {
                connection = new Socket(host, currentPort);
            } catch (ConnectException e) {
                // TODO: bounds checking, don't go upwards forever!
                currentPort += 2;
            } catch (IOException e) {
